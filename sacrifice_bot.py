...
class SacrificeBot:
    ...
    def _board_hash(self, board: chess.Board) -> int:
        ...
        # Newer python-chess builds expose ``Board.zobrist_hash()``, older ones
        # have ``Board.transposition_key`` (method or property) or only the
        # private ``_transposition_key`` attribute.  Probe them in that order so
        # the engine keeps working regardless of which API is available.
