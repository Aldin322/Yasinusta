...
officially started and then immediately attaches to the board stream, so it no
longer misses the acceptance notification when the opponent clicks "Accept"
before the SSE connection is ready.  Your bot therefore stops hanging on
"Waiting for the game to start" and begins calculating its first move as soon as
Lichess starts feeding the board state.
